...
-- priority states (build-order doc §Phase 3 states: Delaware/California/
-- Florida/New York/Texas). Run after 006_filing_provider.sql. See
-- ../schema.sql for the canonical schema.
--
-- `state` values are the FULL state name (e.g. 'Delaware', not 'DE'), to
-- match registrations.state and lib/entities/stateFees.ts's STATE_FEES keys
-- — every state value in this app is the full name end to end (the
-- onboarding dropdown is literally Object.keys(STATE_FEES)). An earlier
-- version of this migration used two-letter codes, which meant
-- getComplianceRule()'s `WHERE state = $1` never matched anything at
-- runtime — every registration silently fell back to the anniversary
-- approximation instead of these "verified" dates. Fixed before this ever
...
('Delaware', 'llc', 'annual_report', 'fixed_date', 'annual', 6, 1,
...
('Delaware', 'all', 'annual_report', 'fixed_date', 'annual', 3, 1,
...
('Florida', 'all', 'annual_report', 'fixed_date', 'annual', 5, 1,
...
('Texas', 'all', 'annual_report', 'fixed_date', 'annual', 5, 15,
...
('California', 'llc', 'annual_report', 'anniversary_month_last_day', 'biennial', NULL, NULL,
...
('California', 'all', 'annual_report', 'anniversary_month_last_day', 'annual', NULL, NULL,
...
('New York', 'all', 'annual_report', 'anniversary_month_last_day', 'biennial', NULL, NULL,
