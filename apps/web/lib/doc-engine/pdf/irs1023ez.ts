...
export async function build1023EZPrefillPdf(O: OrgData): Promise<Buffer> {
  ...
  function frBranding() {
    ...
    doc.setFontSize(7);
    rgb([148, 163, 184]);
    doc.text("Pre-filled by FormRight  |  rightform.org  |  File online at pay.gov", W - 38, 18, { align: "right" });
  }
  ...
  function pageFooter(pageNum: number) {
    ...
    doc.setFont("helvetica", "normal");
    rgb(DGRAY);
    doc.text(`Form 1023-EZ Pre-Fill — Generated by FormRight | rightform.org — Page ${pageNum} of 3`, 36, H - 20);
  ...
  const notes = [
    ...
    "Questions? Contact FormRight at support@rightform.org  |  rightform.org",
  ];
  notes.forEach((note) => {
    doc.setFontSize(6.8);
    doc.setFont("helvetica", "normal");
  ...
  doc.setFont("helvetica", "bold");
  rgb(TEAL);
  doc.text("Generated by FormRight  |  rightform.org  |  hello@rightform.org", W / 2, y + 12, { align: "center" });
  doc.setFontSize(6.5);
  doc.setFont("helvetica", "normal");
  rgb(DGRAY);
